		...
		<#if (cmsSite.recordInformation)?? && cmsSite.recordInformation ?length gt 0>
		    /* 备案信息 */
			and cs.record_information like CONCAT('%', :cmsSite.recordInformation ,'%') 
		</#if>
		<#if (cmsSite.longitude)?? && cmsSite.longitude ?length gt 0>
		    /* 经度 */
			and cs.longitude like CONCAT('%', :cmsSite.longitude ,'%') 
		</#if>
		<#if (cmsSite.latitude)?? && cmsSite.latitude ?length gt 0>
		    /* 纬度 */
			and cs.latitude like CONCAT('%', :cmsSite.latitude ,'%') 
		</#if>
