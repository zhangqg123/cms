		...
		<#if (cmsMenu.owner)?? && cmsMenu.owner ?length gt 0>
		    /* IMAGE_HREF */
			and cm.OWNER like CONCAT('%', :cmsMenu.owner ,'%') 
		</#if>
		...
		<#if (cmsMenu.content)?? && cmsMenu.content ?length gt 0>
		    /* content */
			and cm.content like CONCAT('%', :cmsMenu.content ,'%') 
		</#if>
