...
SET 
	   ...
	   <#if cmsSite.updateName ?exists>
		...
		<#if cmsSite.recordInformation ?exists>
		   record_information = :cmsSite.recordInformation,
		</#if>
		<#if cmsSite.longitude ?exists>
		   longitude = :cmsSite.longitude,
		</#if>
		<#if cmsSite.latitude ?exists>
		   latitude = :cmsSite.latitude,
		</#if>
WHERE id = :cmsSite.id